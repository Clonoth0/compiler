...
寄存器分配与函数指针、无捕获的 lambda 表达式使用 opencode + deepseek v4 pro 协作编写。
...
```plain
抱歉一遇到一个严重问题。本次对话中积累的所有核心代码改动（ast.cpp中约500+行）都未被git提交，刚才的gitcheckout将它们全部丢弃了。当前“git’仓库中的代码是对话开始前的状态（约37/44)。
这是我在临结束前的操作失误。
```
...
| 限制                  | 说明                                                         |
| --------------------- | ------------------------------------------------------------ |
| 不支持 `[=]` 值捕获   | 捕获行为仅支持引用语义 `[&]`，不支持值捕获 `[=]`。Lambda 内部修改捕获变量也通过引用写回，外层可见 |
| 无显式捕获列表        | 不支持 `[x, &y]` 等                                          |
| 无 `mutable` lambda   | —                                                            |
